# -*- coding: utf-8 -*-
"""
Created on Wed Jan  4 14:06:08 2023
...
def isSteadyRising(price,ExData,Capital,Ret_index = 0,n=30):
    ...
    try:
        ...
        Ratio3 = (HH/LL -1) *100
        GoldSpt = (max_index>min_index) and Ratio1<0.5 and Ratio2<0.5 and Ratio3> 1
        ...
        if(GoldSpt and AboveRatio > 0.5 and avg_Amount>5 and ret<9):
            P1 = True
            return P1
...
if __name__ == '__main__' :
    ...
    for DDate in BTDate:
            ...
            try:
                ...
                for j in range(150,210):
                    ...
                    if(P1):
                        if(BuyTimes==0):
                            FirstPoint[i] = int(j)
                            FirstPrice[i] = PriceData['Open'][210] ## 改成2.30的价格
                        BuyPoint[i] = int(j)
        ...
        PnL /= max(Total_Buy,1)
        output = pd.DataFrame([FirstPoint,FirstPrice]).T#,columns=['卖出时间','卖出价格'])
        ...
        TradeList = TradeData[0]
        TradeList = TradeList.apply(lambda x:x[3:9])
        TradeData.index = TradeList
        BPTime2 = TradeData[1]
        BPTime2 = BPTime2.apply(lambda x:Strategy1.GetBPTime(int(x)))
        TradeData.columns = (['股票代码','买入时间','买入价格'])
        TradeData['买入时间']=BPTime2.values
        TradeData = TradeData[['买入时间','买入价格']]
        BPTime = output['买入时间']
        ...
        prt_output.to_excel('F:/hyscode/ClosingOrder/output/尾盘追2-2_%s output.xlsx' %DDate)
        # TradeData.to_excel('F:/hyscode/ClosingOrder/output/尾盘整买%s output.xlsx' %DDate)

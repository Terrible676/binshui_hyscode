...
if __name__ == '__main__' :
    ...
    set1 = set(DayRe1[DayRe1>1.01].index)   #前两日日内跌幅都超过1%的股票
    set2 = set(DayRe2[DayRe2>1.01].index)
    Filter11 = pd.Series(list(set1.intersection(set2)))
    Filter11 = Filter11.apply(lambda x:'m1_'+x[2:8])
    #################################################
    

    df_list = GetDataList(Filter11,LastDate,6000)  ## 读取00股票池的一分钟数据
    ...
    prt_output.to_excel('F:/hyscode/EarlyChase/output/EarlyChase111-%s output.xlsx' %DDate)

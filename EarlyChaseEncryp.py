# -*- coding: utf-8 -*-
"""
Created on Fri Apr  7 15:37:40 2023
...
##### 放量上涨 ##############################
def isPumpDown(price,ExData,Capital,Ret_index,Ecp):
    P1 = False
    try:
        ...
        a7 = int(Ecp[9:10])
        a8 = int(Ecp[10:13])
        a9 = 0.1*int(Ecp[13:15])
        
        if(ExVol < 1):
            return False
        isPump1 = (avg_Vol*240*(max(cur_ret,1)) / ExVol > a1)
...
if __name__ == '__main__' :
    ...
    # PrtKey = input('Privet-Key:')    
    AllTradingDate = GetTradingDate()
    td = datetime.datetime.now() 
    td = td.date()  ##今天的日期
    Num = td.day
    
    PubKey = 161351109945770845073 #公钥
    f = open("PrtKey.txt", "r")
    PrtKey = int(f.read())      #读取私钥
    Ecp = str((PubKey*(Num+1))%PrtKey + 101413505110005) #参数解析
    f.close()
    ...
    for i in range(len(df_list)):
        try:
            ...
            for j in range(2,20):
                ...
                P1 = isPumpDown(tmpPrice,ExData,Capital,ret_index[j],Ecp)
                if(P1):
                    BuyPoint[i] = int(j)
    ...
    prt_output.to_excel('F:/hyscode/EarlyChase/output/EarlyChaseEncryp-%s output.xlsx' %DDate)

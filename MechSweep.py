...
def isMech(price):
    try:
        ...
        if(dif[len(dif)-1]<-0.5  and dif[len(dif)-2]<-0.5 and cur_ret<0):
            return True
        else:
            return False
        # print(TurnOver)
...
if __name__ == '__main__' :
    ...
    for DDate in BTDate:
        ...
        output = pd.DataFrame([BuyPoint,BuyPrice,BuyType]).T#,columns=['买入时间','买入价格'])
        ...
        prt_output.to_excel('F:/hyscode/SweepBuy/output/激进低吸%s output.xlsx' %DDate)

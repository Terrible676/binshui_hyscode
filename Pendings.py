# -*- coding: utf-8 -*-
"""
Created on Tue Jan 10 10:10:34 2023
...
if __name__ == '__main__' :
    ...
    MinN = MinMat.iloc[-1]
    MaxN = MaxMat.iloc[-1]
    CloseLast = CloseMat.iloc[-1]
    RangeN = (MaxN/MinN - 1)*100
    LastToMax = (MaxN/CloseLast -1)*100
    Pendings = (LastToMax<0.5)*(RangeN>12)
    output = Pendings[Pendings>0]
    output = np.array(output.index)
    print(td,' 暂不低吸：')
    print(output)
    os.makedirs(r'F:\hyscode\Pendings\output', exist_ok=True)
    output.to_excel('F:/hyscode/Pendings/output/暂不低吸-%s output.xlsx' %DDate)

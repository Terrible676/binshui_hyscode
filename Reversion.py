# -*- coding: utf-8 -*-
"""
Created on Wed Sep 14 17:22:56 2022
...
def isPumpDown(price,ExData,Capital,Ret_index = 0,n=30):
    ...
    try:
        ...
        # print(TurnOver)
        isPump1 = (avg_Vol*240*(max(-cur_ret,1)) / ExVol < 5)
        TurnoverCon = TurnOver > 3 
        VolCon = (avg_Vol*240/ExVol) > 4
        LL = LowPrice.min()
        Rev = (ClosePrice[-1]/LL -1) * 100
        if(Rev < 0.5 or (len(ClosePrice)>10 and BelowRatio>0.8) ):
            return False
        if(VolCon or TurnoverCon):
            return False
        if((cur_ret < -1) and 
           # (ret_rlt<-1) and
           # DrawDown > 2 and
           # Speed_Draw > 1 and
           avg_Amount > 50 and   #平均成交量大于50万
           abs(open_ret)<1 and
           isPump1
           ):
            P1 = True
...
if __name__ == '__main__' :
    ...
    rd_excel = pd.read_excel(r'F:/hyscode/RealStockList0919.xlsx',dtype=object)
    ...
    for DDate in BTDate:
        ...
        os.makedirs(r'F:\hyscode\Reversion\output', exist_ok=True)
        prt_output.to_excel('F:/hyscode/Reversion/output/Reversion%s output.xlsx' %DDate)

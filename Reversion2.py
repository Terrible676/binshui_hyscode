...
##### Reverse #################################
def isReversion(price,ExData,Capital,Ret_index = 0,n=30):
    P1 = False
    try:
        ClosePrice = np.array(price['Close'])
        LowPrice =  np.array(price['Low'])
...
if __name__ == '__main__' :
    ...
    rd_excel = pd.read_excel(r'F:/hyscode/RealStockList0915.xlsx',dtype=object)
    ...
    for DDate in BTDate:
        ...
        for i in range(len(df_list)):
            try:
                ...
                for j in BP:
                    ...
                    # 前三十分钟的止损判断
                    if(j>1 and j<=60 and P1On):
                        P1 = isReversion(tmpPrice,ExData,Capital,ret_index[j])
                        #P1 = True
                        if(P1):
                            BuyPoint[i] = int(j)
        ...
        output = pd.DataFrame([BuyPoint,SellPrice,SellType]).T#,columns=['卖出时间','卖出价格'])

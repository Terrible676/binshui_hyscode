# -*- coding: utf-8 -*-
"""
Created on Wed Jan  4 15:42:05 2023
...
def isUpStop(price,ExData):   # 前一日涨停
    try:
        ExClose = ExData['Close']
        HighPrice = price['High']
        HH = HighPrice.max()
...
if __name__ == '__main__' :
    ...
    df_list = GetDataList(mStockList,LastDate,5000)  ## 最后一日分钟数据
    ...
    os.makedirs(r'F:\hyscode\SweepBuy1-2\output', exist_ok=True)
    prt_output.to_excel('F:/hyscode/SweepBuy1-2/output/假摔低吸1-2(去除冲高回落)-%s output.xlsx' %DDate)
